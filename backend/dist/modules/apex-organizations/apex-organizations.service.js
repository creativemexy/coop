...
var ApexOrganizationsService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.ApexOrganizationsService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const bcrypt = __importStar(require("bcrypt"));
const apex_organization_entity_1 = require("./entities/apex-organization.entity");
...
let ApexOrganizationsService = ApexOrganizationsService_1 = class ApexOrganizationsService {
    repo;
    userRepo;
    emailService;
    smsService;
    paystackClient;
    logger = new common_1.Logger(ApexOrganizationsService_1.name);
    ...
    async ensureUniqueCode(base) {
        ...
        while (await this.repo.findOne({ where: { code } })) {
            ...
            if (attempts > 50) {
                code = `APEX${Date.now().toString(36).toUpperCase()}`;
                break;
            }
        }
        return code;
    ...
    async create(dto) {
        ...
        await this.emailService.send({
            to: recipientEmail,
            subject: `Apex Organization Created — ${dto.name}`,
            text: [
                `Dear ${dto.contactPersonName || 'Apex BM'},`,
                '',
                `Your apex organization "${dto.name}" has been created successfully.`,
                '',
        ...
        await this.smsService.send((0, temp_password_util_1.normalizePhoneForSms)(phone), [
            `Your apex organization "${dto.name}" (code ${code}) has been created.`,
            `Login: ${email}`,
            `Temporary password: ${password}`,
            'Change it on first login. - Coop BNPL',
        ].join('\n'), 'apex_organization_created');
        this.logger.log(`Apex org created: ${code}, BM user: ${(0, mask_util_1.maskEmail)(email)}`);
    ...
    async findById(id) {
        ...
        if (!org) {
            throw new common_1.NotFoundException('Apex organization not found');
        }
        return org;
    }
    async update(id, dto) {
...
exports.ApexOrganizationsService = ApexOrganizationsService;
exports.ApexOrganizationsService = ApexOrganizationsService = ApexOrganizationsService_1 = __decorate([
...
], ApexOrganizationsService);
//# sourceMappingURL=apex-organizations.service.js.map