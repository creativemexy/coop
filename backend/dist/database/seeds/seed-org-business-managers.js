...
exports.seedOrgBusinessManagers = seedOrgBusinessManagers;
...
async function seedOrgBusinessManagers(dataSource) {
    ...
    for (const org of orgs) {
        const existing = await userRepo.findOne({
            where: { role: role_enum_1.Role.BUSINESS_MANAGER, organizationId: org.id },
        });
        if (existing) {
        ...
        await userRepo.save({
            ...
            emailHash: (0, encryption_service_1.hashForLookup)(`bm-${slug}@coop.com`),
            passwordHash,
            firstName: 'Business',
    ...
    if (seeded === 0)
        console.log('All org BMs already exist, skipping');
}
//# sourceMappingURL=seed-org-business-managers.js.map