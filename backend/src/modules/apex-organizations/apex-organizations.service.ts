...
@Injectable()
export class ApexOrganizationsService {
  private readonly logger = new Logger(ApexOrganizationsService.name);

  constructor(
    @InjectRepository(ApexOrganization)
    private readonly repo: Repository<ApexOrganization>,
  ...
  async create(dto: {
    ...
    sortCode?: string;
    bankCode?: string;
  }): Promise<ApexOrganization & { generatedEmail?: string; generatedPassword?: string }> {
    ...
    // Create apex business manager user
    ...
    this.logger.log(`Apex org created: ${code}, BM user: ${maskEmail(email)}`);

    return { ...saved, generatedEmail: email, generatedPassword: password } as any;
  }

  async findAll(): Promise<ApexOrganization[]> {
  ...
  async findById(id: string): Promise<ApexOrganization> {
    const org = await this.repo.findOne({
      where: { id },
      relations: { organizations: true },
    });
    if (!org) {
      throw new NotFoundException('Apex organization not found');
    }
    return org;
  }
