...
export function Organizations() {
  ...
  const fetch = useCallback(() => {
    ...
    api.get('/apex-organizations').then((r) => setApexOrgs(r.data))
  }, [])
  ...
  const handleCreate = async () => {
    setSaving(true)
    try {
      const { data } = await api.post('/organizations', { ...form, address: composeAddress(form) })
      setCreatedResult({
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true)
    try {
      await api.patch(`/organizations/${selected.id}`, { ...form, address: composeAddress(form) })
      setEditOpen(false)
      setSelected(null)
      setForm(emptyForm())
      fetch()
  ...
  const openEdit = (o: Org) => {
    ...
    setForm({
      name: o.name, apexOrgId: o.apexOrgId,
  ...
  const resolveAccountName = async (accountNumber: string, bankCode: string) => {
    ...
    setResolving(true)
    try {
      const { data } = await api.get('/organizations/bank/resolve', {
  ...
  const requiredFields: (keyof OrgForm)[] = [
    'name', 'apexOrgId', 'houseNumber', 'streetName', 'city', 'state',
    'contactEmail', 'contactPhone',
    'contactPersonName', 'contactPersonPhone', 'contactPersonEmail',
    'bankName', 'accountName', 'accountNumber', 'sortCode', 'bankCode',
  ]
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold dark:text-gray-100">Organizations</h2>
        <Button onClick={openCreate}>Create</Button>
      </div>

      <Input placeholder="Search by name or code..." value={search} onChange={(e) => setSearch(e.target.value)} />

      <Card className="border-teal-500 p-0 overflow-hidden">
        <Table>
          <THead>
            <THeadRow>
              <THeadCell>Name</THeadCell>
              <THeadCell>Code</THeadCell>
              <THeadCell>Apex Org</THeadCell>
              <THeadCell>Status</THeadCell>
              <THeadCell>Contact</THeadCell>
          ...
          <TBody>
            {filtered.map((o) => (
              <TBodyRow key={o.id}>
                ...
                <TBodyCell>{o.apexOrg?.name ?? '—'}</TBodyCell>
                <TBodyCell><Badge variant={o.status === 'active' ? 'success' : 'danger'}>{o.status}</Badge></TBodyCell>
                <TBodyCell className="text-xs">{o.contactEmail || o.contactPhone || '—'}</TBodyCell>
            ...
            {filtered.length === 0 && (
              <TBodyRow>
                <TBodyCell colSpan={7} className="text-center text-gray-400 py-8">No organizations found</TBodyCell>
              </TBodyRow>
            )}
          </TBody>
        </Table>
            ...
            {step === 1 && (
              <>
                ...
                <div className="space-y-1">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Apex Organization</label>
                  <select
                    value={form.apexOrgId}
                    onChange={(e) => setForm({ ...form, apexOrgId: e.target.value })}
                    ...
                    required
                  >
                    <option value="">Select apex org</option>
      ...
      <Modal open={!!selected && !editOpen} onClose={() => setSelected(null)} title={selected?.name ?? ''}>
        {selected && (
          <div className="space-y-4">
            ...
            <div><span className="text-sm text-gray-500">Apex Org:</span><p className="font-medium">{selected.apexOrg?.name ?? '—'}</p></div>
      ...
      <Modal open={!!createdResult} onClose={() => setCreatedResult(null)} title="Organization Created">
        {createdResult && (
          <div className="space-y-4">
            <p className="text-sm text-green-600 font-medium">Organization created successfully!</p>
            <p className="text-sm text-gray-500">An email has been sent with the login details below.</p>
